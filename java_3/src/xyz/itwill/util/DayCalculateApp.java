...
public class DayCalculateApp {
	public static void main(String[] args) {
		...
		try {		
			Date bir = dateFormat.parse(birthday);
			Date now = new Date();
		         
			long difference = now.getTime() - bir.getTime();
			
            System.out.println("[결과]태어난지 <" +difference/(1000 * 86400)+ "일>이 지났습니다.");
        } catch (ParseException e) {
            System.out.println("[에러]형식에 맞는 생년월일을 입력해 주세요.");
            System.exit(0);
        }finally {		
        	scanner.close();
        }
